...
namespace Storage.Controllers
{
    public class ProductsController : Controller
    {
        ...
        // GET: Products/ProductViewModel/5
        public async Task<IActionResult> ProductsView()
        {
            var categories = _context.Product.Select(c => c.Category);
            var products = await _context.Product.ToListAsync();
            var cats=products.Select(c => c.CategoryEnum);
            //var total =  _context.Product.Sum(x => x.Price);
        ...
        public async Task<IActionResult> ProductsFilter(string Category)
        {
            List<ProductViewModel> viewModel;

            if (string.IsNullOrEmpty(Category))
            {
                viewModel = await _context.Product
            ...
            {
                viewModel = await _context.Product
                .Where(e => e.Category.StartsWith(Category))
                .Select(e => new ProductViewModel
                {
                    Name = e.Name,
                    Price = e.Price,
            ...
            return View(nameof(ProductsView), viewModel);
        }
